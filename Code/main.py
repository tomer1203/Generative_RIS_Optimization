...
@timeit
def annealed_langevin_v3(model,physfad,starting_configuration,tx_x,tx_y, number_of_iterations = 5,epsilon=torch.tensor(0.01),device=torch.device("cpu")):
    ...
    for i in range(100000):
        ...
        if i % 60 == 0 and i != 0:
            ...
            for batch in test_ldr:
                ...
                ald_results = annealed_langevin_v3(model_diffusion, physfad, X, tx_x, tx_y, epsilon=config.diffusion_epsilon,
                                                device=device)
...
def diffusion_active_training(model_diffusion,test_ldr,optimizer_diffusion, physfad, config, device=torch.device("cpu")):
    ...
    for i in range(1000):
        ...
        grad_inp_64 = zo_estimate_gradient(capacity_physfad, improved_X , tx_x, tx_y, epsilon, 64, device,broadcast_tx=False)
        acc_grads = get_physfad_grads(improved_X,tx_x,tx_y,physfad,device,noise=None,broadcast_tx=False)
        grad_inp = grad_inp_64
        print(cosine_score(grad_inp_64,acc_grads))
        ...
        if i % config.ep_log_interval == 0 and i != 0:
            model_diffusion.eval()
            torch.save(model_diffusion.state_dict(), "./Models/Full_Main_model_with_box.pt")
            acc_rate = 0
            ...
            for batch in test_ldr:
                ...
                X = X[0:batch_size]
                # improved_X = model_diffusion(torch.hstack([X,tx_x, tx_y,torch.tensor(1,device=device).unsqueeze(0).unsqueeze(0)]))
                ald_configuration_results = annealed_langevin_v3(model_diffusion, physfad, X, tx_x, tx_y, epsilon=config.diffusion_epsilon,
                                                device=device)

                for i, (iter, ris_configuration) in enumerate(ald_configuration_results):
                    capac, _ = test_configurations_capacity(physfad, ris_configuration, tx_x, tx_y, device, list_out=False)
        ...
        model_diffusion.train()
        # print(rate.item())
def diffusion_inference(model_diffusion,model_diffusion_box,test_ldr, physfad, config, device="cpu"):
    output_graph = np.zeros([4,50])
    output_graph_box = np.zeros([4,50])
    ald_capacity_avg = np.zeros(50)
    ald_capacity_avg_box = np.zeros(50)
    ald_iter_list = np.zeros(50)
    ald_iter_list_box = np.zeros(50)
    zogd_capacity_avg = np.zeros(50)
    physfad_capacity_avg = np.zeros(50)
    ...
    for batch_idx,batch in enumerate(test_ldr):
        ...
        ald_configuration_results = annealed_langevin_v3(model_diffusion, physfad, X, tx_x, tx_y,epsilon=config.diffusion_epsilon, device=device)
        ald_configuration_results_box = annealed_langevin_v3(model_diffusion_box, physfad, X, tx_x, tx_y,epsilon=config.diffusion_epsilon, device=device)
        for i, (iter, ris_configuration) in enumerate(ald_configuration_results):
            capac,_ = test_configurations_capacity(physfad, ris_configuration, tx_x, tx_y, device, list_out=False)
            # capac = breakdown(ris_configuration,tx_x,tx_y,physfad,device)
            print(i,capac)
            ...
            ald_iter_list[i] = iter
        for i, (iter, ris_configuration) in enumerate(ald_configuration_results_box):
            capac,_ = test_configurations_capacity(physfad, ris_configuration, tx_x, tx_y, device, list_out=False)
            # capac = breakdown(ris_configuration,tx_x,tx_y,physfad,device)
            print(i,capac)
            ald_capacity_avg_box[i] += capac
            output_graph_box[0,i] += capac
            ald_iter_list_box[i] = iter
        ## Testing Physfad on clean environment
        physfad.save_and_change_to_clean_environment() # Load the clean environment and reset the bessel memory
        (_, physfad_capacity, _, physfad_ris_configurations) = (
        ...
        ## Testing the same physfad configurations on the noisy environment
        physfad.reload_original_environment() # Load the original environment and reset the bessel memory
        for i, capac in enumerate(physfad_capacity):
            capacity,_ = test_configurations_capacity(physfad, physfad_ris_configurations[i], tx_x, tx_y, device, list_out=False)
    ...
    plt.plot(ald_iter_list_box, ald_capacity_avg_box / (120*(batch_idx+1)),'-*',linewidth=2)#,linestyle='dashed')
...
def room_visualization(net_diffusion, test_ldr, physfad, config, device, optimized=True):
    ...
    for i in range(x_sample_density):
        ...
        for j in range(y_sample_density):
            ...
            room_image[i, j] = test_configurations_capacity(physfad, ris_configuration, tx_x, tx_y, device, list_out=False)[0]
            idx = idx + 1
...
def room_graph_print(physfad):
    ...
    plt.show()
def find_optimal_learning_rate(physfad,test_ldr,config,device):
    device_cpu = torch.device('cpu')


    (X, _, _, _, _, _) = open_virtual_batch(next(iter(test_ldr)))  # (predictors, targets)
    # we test the learning rate on the default tx location
    tx_x, tx_y = physfad.generate_tx_location(1, device, modify=False)
    ...
    plt.show()
def snr_graph(physfad, test_ldr, net_diffusion, config, device):
    device_cpu = torch.device('cpu')
    ...
    for attempt in range(attempts):
        ...
        for i, noise in enumerate(noise_array):
            ...
            (physfad_time_lst, physfad_capacity, last_configuration, physfad_inputs) = physfad_channel_optimization(
                device_cpu, physfad,
                copy_with_gradients(
                    initial_inp,
                    device_cpu),
            ...
            for j, capac in enumerate(physfad_capacity):
                physfad_capacity_avg_with_noise[j], _ = test_configurations_capacity(physfad, physfad_inputs[j], tx_x,
                                                                                     tx_y, device,
            ...
            dnn_physfad_capacity_lst = test_dnn_optimization(physfad, [x[1].cpu() for x in ald_configuration_results],
...
def main():
    ...
    net_diffusion_box = Net_diffusion(config).to(device)
    ...
    if load_model:
        ...
        net_diffusion_box.load_state_dict(torch.load("./Models/Full_Main_model_with_box.pt"))
        optimizer_diffusion = T.optim.Adam(net_diffusion.parameters(), lr=lrn_rate)

    if diffusion_train_mode:
        print("Training Diffusion Model")
        diffusion_active_training(net_diffusion, test_ldr, optimizer_diffusion, physfad, config, device=device)

    if find_optimal_lr:
        print("Finding Best Learning Rate for Simulation")
        find_optimal_learning_rate(physfad,test_ldr,config,device)

    if test_models_optimization_per_iteration:
        print("Testing models optimization per iteration")
        diffusion_inference(net_diffusion, net_diffusion_box, test_ldr, physfad, config, device=device)

    if run_snr_graph:
        print("Running SNR graph")
        snr_graph(physfad, test_ldr, net_diffusion, config, device)

    if run_room_visualization:
        print("Generating room visualization")
        room_visualization(net_diffusion,test_ldr,physfad,config,device,optimized=False)
        room_visualization(net_diffusion,test_ldr,physfad,config,device,optimized=True)
